export function hexLayout(map, width, height, margin = 12) {
  ...
  const size = Math.min(
    (width - margin * 2) / (horizontal * (map.columns + 0.5)),
    (height - margin * 2) / (1.5 * (map.rows - 1) + 2),
  );
  return {
    size,
    left: (width - horizontal * size * (map.columns + 0.5)) / 2,
...
export function traceHex(context, geometry) {
  context.beginPath();
