...
export function evaluateReleaseGate(evidence={}){
  ...
  const acceptance=RELEASE_GATE.lobbySizes.every(size=>{const matches=acceptanceArtifacts.filter(x=>x.lobbySize===size),ids=matches[0]?.scenarioIds;return matches.length===1&&Array.isArray(ids)&&ids.length===expected.length&&new Set(ids).size===ids.length&&ids.every((id,index)=>id===expected[index]);});
